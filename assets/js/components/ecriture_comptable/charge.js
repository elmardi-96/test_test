...
var table = $("#grid").DataTable({
    ajax: {
      url: Routing.generate("ecriture_comptable_compta_charge_list"), // json datasource
      type: "get",
    },
    deferRender: true,
    stateSave: true,
...
$("body").on('keypress', '#id_piece', function(e) {
    ...
    if(e.which == 13)  {
        ...
        $.ajax({
            type: "GET",
            url: Routing.generate("ecriture_comptable_operation_getecriturecomptable", {"idPiece": idPiece.trim(), "type" : "faf"}),
            processData: false,
            contentType: false,
...
function removeFactureFromTable() {
    ...
    $.ajax({
        type: "GET",
        url: Routing.generate("ecriture_comptable_charge_flag_facture", {"idPiece": idPiece.trim()}),
        processData: false,
        contentType: false,
        success: function (result) {
