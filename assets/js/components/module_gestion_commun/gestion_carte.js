...
var tr_alimentation_list_index = $('#grid_index_depot_retrait').DataTable({
    ajax: {
        url: Routing.generate('compte_carte_list'), // json datasource
        type: "get",
    },
    "deferRender": true,
    "stateSave": false,
...
$("body").on('click' ,".new_operation_button", function() {
    ...
    $.ajax({
        type: "POST",
        url: Routing.generate("compte_carte_new", {"dossier": id}),
        processData: false,
        contentType: false,
...
$("body").on("click", "#create_caisse", function (e) {
    ...
    $.ajax({
        type: "POST",
        url: Routing.generate("compte_carte_store"),
        data: formData,
        processData: false,
        contentType: false,
        success: function (result) {
...
$("body").on("keyup", "#u_general_operation_montant", function(){
    ...
    if((montant_initial - montant.val()) < 0) {
        ...
        toastr.warning("Vous passer le montant que vous aves dans la carte !", "warning", {timeOut: 4000})
