...
var table = $('#grid').DataTable({
    ajax: {
        url: Routing.generate('p_marche_sous_list'), // json datasource
        type: "get",
    },
    "deferRender": true,
    "stateSave": true,
...
table.on("init.dt", function (e, settings) {
  ...
  CustomSearchDatatable.CustomSearchText(api, [1,3,4,5,8], []);
  CustomSearchDatatable.CustomSearchSelect(api, [2,9], []);
  CustomSearchDatatable.CustomSearchDate(api, [6,7], []);
...
$(".add").click(function (e) {
  ...
  //alert();
    $.ajax({
      type: "POST",
      url: Routing.generate("p_marche_sous_new"),
  ...
  $(".bod").on("submit", ".form", function (e) {
    ...
    if ($(document.activeElement).hasClass("button-save-add-marche")) {
        //alert()
      var l = Ladda.create(document.querySelector(".button-save-add-marche"));
      var goto = 1;
    } else {
      var l = Ladda.create(document.querySelector(".button-save-marche"));
      var goto = 0;
    }
    l.start();
  ...
  $("#grid tbody").on("click", ".cd_op", function () {
    $.ajax({
      type: "POST",
      url: Routing.generate("p_marche_sous_edit", {
  ...
  $(".bod").on("submit", ".form_edit", function (e) {
    ...
    var id = $("#test").data("marcheId");
    var l = Ladda.create(document.querySelector(".button-edit-marche"));
    l.start();
    var obj = new Object();
    var parametre = { id: id };
