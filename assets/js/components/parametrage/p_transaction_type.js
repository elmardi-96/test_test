...
var table = $('#grid').DataTable({
    ajax: {
        url: Routing.generate('p_transaction_type_list'), // json datasource
        type: "get",
    },
    "deferRender": true,
    "stateSave": true,
...
$(".add").click(function (e) {
    $.ajax({
      type: "POST",
      url: Routing.generate("p_transaction_type_new"),
  ...
  $(".bod").on("submit", ".form", function (e) {
    ...
    if ($(document.activeElement).hasClass("button-save-add-transaction")) {
        //alert()
      var l = Ladda.create(document.querySelector(".button-save-add-transaction"));
      var goto = 1;
    } else {
      var l = Ladda.create(document.querySelector(".button-save-transaction"));
      var goto = 0;
    }
    l.start();
  ...
  $("#grid tbody").on("click", ".cd_op", function () {
    $.ajax({
      type: "POST",
      url: Routing.generate("p_transaction_type_edit", {
  ...
  $(".bod").on("submit", ".form_edit", function (e) {
    ...
    var l = Ladda.create(document.querySelector(".button-edit-transaction"));
    l.start();
    var obj = new Object();
    var parametre = { id: id };
