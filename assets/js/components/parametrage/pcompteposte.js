...
var table = $('#grid').DataTable({
    ajax: {
        url: Routing.generate('p_compte_poste_list'), // json datasource
        type: "get",
    },
    "deferRender": true,
    "stateSave": true,
...
$('#grid tbody').on('click', '.cd_op', function () {
    var url = Routing.generate('p_compte_poste_edit', { 'id': $(this).closest('tr').attr('id') });
    ...
    if (id) {
        swalWithBootstrapButtons.fire({
            ...
            if (result.value) {
                $.ajax({
                    type: 'DELETE',
                    url: Routing.generate('p_compte_poste_delete', { 'id': id }),
                    success: function (result) {
                        ...
                        if (result.code == 200) {
                            ...
                            var url = Routing.generate('p_compte_poste_index');
                            window.location.href = url;
