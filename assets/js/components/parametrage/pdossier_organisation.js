...
var table = $('#grid').DataTable({
    ajax: {
        url: Routing.generate('p_dossier_organisation_list'), // json datasource
        type: "get",
    },
    "deferRender": true,
    "stateSave": true,
...
$(".add").click(function (e) {
    $.ajax({
      type: "POST",
      url: Routing.generate("p_dossier_organisation_new"),
  ...
  $(".bod").on("submit", ".form", function (e) {
    ...
    if ($(document.activeElement).hasClass("button-save-add-organisation")) {
        //alert()
      var l = Ladda.create(document.querySelector(".button-save-add-organisation"));
      var goto = 1;
    } else {
      var l = Ladda.create(document.querySelector(".button-save-organisation"));
      var goto = 0;
    }
    l.start();
  ...
  $("#grid tbody").on("click", ".cd_op", function () {
    $.ajax({
      type: "POST",
      url: Routing.generate("p_dossier_organisation_edit", {
  ...
  $(".bod").on("submit", ".form_edit", function (e) {
    ...
    var l = Ladda.create(document.querySelector(".button-edit-organisation"));
    l.start();
    var obj = new Object();
    var parametre = { id: id };
