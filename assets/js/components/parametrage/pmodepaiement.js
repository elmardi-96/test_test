...
var table = $('#grid').DataTable({
    ajax: {
        url: Routing.generate('p_modepaiement_list'), // json datasource
        type: "get",
    },
    "deferRender": true,
    "stateSave": true,
...
$(".add").click(function (e) {
    $.ajax({
      type: "POST",
      url: Routing.generate("p_modepaiement_new"),
  ...
  $(".bod").on("submit", ".form", function (e) {
    ...
    if ($(document.activeElement).hasClass("button-save-add-modepaiement")) {
        //alert()
      var l = Ladda.create(document.querySelector(".button-save-add-modepaiement"));
      var goto = 1;
    } else {
      var l = Ladda.create(document.querySelector(".button-save-modepaiement"));
      var goto = 0;
    }
    l.start();
  ...
  $("#grid tbody").on("click", ".cd_op", function () {
    $.ajax({
      type: "POST",
      url: Routing.generate("p_modepaiement_edit", {
  ...
  $(".bod").on("submit", ".form_edit", function (e) {
    ...
    var id = $("#test").data("modepaiementId");
    var l = Ladda.create(document.querySelector(".button-edit-modepaiement"));
    l.start();
    var obj = new Object();
    var parametre = { id: id };
