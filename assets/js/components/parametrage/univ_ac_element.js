...
var table = $('#grid').DataTable({
    ajax: {
        url: Routing.generate('univ_ac_element_list') + location.search, // json datasource
        type: "get",
    },
    "deferRender": true,
    "stateSave": true,
...
//}
    ...
    CustomSearchDatatable.CustomSearchText(api, [1,7], []);
    CustomSearchDatatable.CustomSearchSelect(api, [2,3,4,5 ,6,8,9,10], []);
...
$(".bod").on("click", "._delete", function (e) {
    ...
    if (id) {
      var obj = new Object();
      obj["route"] = "univ_ac_element_delete";
      obj["id"] = id;
      obj["token"] = token;
  ...
  $("#grid tbody").on("click", ".cd_op", function () {
    $.ajax({
      type: "POST",
      url: Routing.generate("univ_ac_element_edit", {
  ...
  $(".bod").on("submit", ".form_edit", function (e) {
    ...
    var id = $("#test").data("elementId");
   
    var l = Ladda.create(document.querySelector(".button-edit-element"));
    
    l.start();
    var obj = new Object();
    var parametre = { id: id };
    obj["route"] = "univ_ac_element_edit";
    obj["data"] = dta;
  ...
  $(".add").click(function (e) {
    $.ajax({
      type: "POST",
      url: Routing.generate("univ_ac_element_new"),
      success: function (result) {
        $(".bod").html(result).hide();
        $("#ModalDocumentDynamique").modal("show");
        $('select.select2').select2();
  ...
  $(".bod").on("submit", ".form", function (e) {
    var form_name = $(this).attr("name");
    var dta = new FormData(this);
    var selector = $(this);       // alert();
    if ($(document.activeElement).hasClass("button-save-add-element")) {

      var l = Ladda.create(document.querySelector(".button-save-add-element"));
      var goto = 1;
    } else {
      var l = Ladda.create(document.querySelector(".button-save-element"));
      var goto = 0;
    }
    l.start();
    var id = $("#test").data("elementId");
    var obj = new Object();
      obj["route"] = "univ_ac_element_new";
      obj["data"] = dta;
      obj["parametre"] = null;
      obj["l"] = l;
      obj["form_name"] = form_name;
...
$('body').on('change','.promotion',function (e) {
    if ($.isNumeric($(this).val())) {
        ...
        $.ajax({
            url: Routing.generate('univ_ac_etablissement_get_semestre', {id: $(this).val()}),
            success: function (result) {
                var infos2 = jQuery.parseJSON(result);
...
$('body').on('change','.semestre',function (e) {
    if ($.isNumeric($(this).val())) {

       

        $.ajax({
            url: Routing.generate('univ_ac_etablissement_get_module', {id: $(this).val()}),
            success: function (result) {
                var infos2 = jQuery.parseJSON(result);
               
                $(".module_select").html(infos2.data);
                $('select.select2').select2();
            }
        });
        ...
        $('.module_select').html('<option value="">Choix Module</option>');
