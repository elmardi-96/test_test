...
var table = $('#grid').DataTable({
    ajax: {
        url: Routing.generate('univ_ac_formation_list') + location.search, // json datasource
        type: "get",
    },
    "deferRender": true,
    "stateSave": true,
...
//}
    ...
    CustomSearchDatatable.CustomSearchText(api, [1,3,4,5], []);
    CustomSearchDatatable.CustomSearchSelect(api, [2,6], []);
...
$(".bod").on("click", "._delete", function (e) {
    ...
    if (id) {
      var obj = new Object();
      obj["route"] = "univ_ac_formation_delete";
      obj["id"] = id;
      obj["token"] = token;
...
$("#grid tbody").on("click", ".cd_op", function () {
    $.ajax({
      type: "POST",
      url: Routing.generate("univ_ac_formation_edit", {
  ...
  $(".bod").on("submit", ".form_edit", function (e) {
    ...
    var l = Ladda.create(document.querySelector(".button-edit-formation"));
    
    l.start();
    var obj = new Object();
    var parametre = { id: id };
    obj["route"] = "univ_ac_formation_edit";
    obj["data"] = dta;
  ...
  $(".add").click(function (e) {
    $.ajax({
      type: "POST",
      url: Routing.generate("univ_ac_formation_new"),
      success: function (result) {
        $(".bod").html(result).hide();
        $("#ModalDocumentDynamique").modal("show");
        $('select.select2').select2();
  ...
  $(".bod").on("submit", ".form", function (e) {
    ...
    var dta = new FormData(this);
    var selector = $(this);
    if ($(document.activeElement).hasClass("button-save-add-formation")) {
        //alert()
      var l = Ladda.create(document.querySelector(".button-save-add-formation"));
      var goto = 1;
    } else {
      var l = Ladda.create(document.querySelector(".button-save-formation"));
      var goto = 0;
    }
    l.start();
    var id = $("#test").data("formationId");
    var obj = new Object();
      obj["route"] = "univ_ac_formation_new";
      obj["data"] = dta;
      obj["parametre"] = null;
      obj["l"] = l;
      obj["form_name"] = form_name;
