...
var table = $('#grid').DataTable({
    ajax: {
        url: Routing.generate('univ_ac_promotion_list') + location.search, // json datasource
        type: "get",
    },
    "deferRender": true,
    "stateSave": true,
...
$('.add').click(function (e) {
    ...
    $.ajax({
        type: 'POST',
        url: Routing.generate('univ_ac_promotion_ajouter'),
        success: function (result) {
...
$('.bod').on('submit', '.form', function (e) {
    ...
    var selector = $(this);
    if ($(document.activeElement).hasClass("button-save-add-promotion")) {
        var l = Ladda.create(document.querySelector(".button-save-add-promotion"));
        var goto = 1;
      } else {
        var l = Ladda.create(document.querySelector(".button-save-promotion"));
        var goto = 0;
      }
    l.start();
...
$('#grid tbody').on('click', '.cd_op', function () {
           ...
           $.ajax({
               type: 'POST',
               url: Routing.generate('univ_ac_promotion_modifier',{ 'id': $(this).closest('tr').attr('id')}),
               success: function (result) {
                   $('.bod').html(result).hide()
                   $('#ModalDocumentDynamique').modal('show');
                   $('select.select2').select2();
   ...
   $('.bod').on('submit', '.form_edit', function (e) {
    ...
    var id = $('#test').data("promotionId");
    var l = Ladda.create(document.querySelector(".button-edit-promotion"));
    l.start();
    $.ajax({
        type: 'POST',
        url: Routing.generate('univ_ac_promotion_modifier',{ 'id': id}),
        data: dta,
        processData: false,
        contentType: false,
        success: function (result) {
    ...
    if (id) {
        swalWithBootstrapButtons.fire({
            ...
            if (result.value) {
                $.ajax({
                    type: 'DELETE',
                    url: Routing.generate('univ_ac_promotion_delete', { 'id': id , 'token': token}),
                    success: function (result) {
                      //  console.log(result);
                        if (result.code == 200) {
                            // table.ajax.reload();
