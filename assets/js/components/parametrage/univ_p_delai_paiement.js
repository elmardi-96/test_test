...
var table = $('#grid').DataTable({
    ajax: {
        url: Routing.generate('univ_p_delai_paiement_list') + location.search, // json datasource
        type: "get",
    },
    "deferRender": true,
    "stateSave": true,
...
table.on("init.dt", function (e, settings) {
  ...
  CustomSearchDatatable.CustomSearchText(api, [1,4], []);
  CustomSearchDatatable.CustomSearchSelect(api, [2,3,7], []);
  CustomSearchDatatable.CustomSearchDate(api, [5,6], []);
...
$(".bod").on("click", "._delete", function (e) {
    ...
    if (id) {
      var obj = new Object();
      obj["route"] = "univ_p_delai_paiement_delete";
      obj["id"] = id;
      obj["token"] = token;
...
$(".bod").on("submit", ".form", function (e) {
    ...
    var selector = $(this);
    if ($(document.activeElement).hasClass("button-save-add-paiement")) {
        //alert()
      var l = Ladda.create(document.querySelector(".button-save-add-paiement"));
      var goto = 1;
    } else {
      var l = Ladda.create(document.querySelector(".button-save-paiement"));
      var goto = 0;
    }
    l.start();
    var id = $("#test").data("paiementId");
    var obj = new Object();
      obj["route"] = "univ_p_delai_paiement_add";
      obj["data"] = dta;
      obj["parametre"] = null;
      obj["l"] = l;
...
$(".add").click(function (e) {
    $.ajax({
      type: "POST",
      url: Routing.generate("univ_p_delai_paiement_add"),
  ...
  $("#grid tbody").on("click", ".cd_op", function () {
    $.ajax({
      type: "POST",
      url: Routing.generate("univ_p_delai_paiement_edit", {
  ...
  $(".bod").on("submit", ".form_edit", function (e) {
    ...
    var id = $("#test").data("paiementId");
    var l = Ladda.create(document.querySelector(".button-edit-paiement"));
    l.start();
    var obj = new Object();
    var parametre = { id: id };
    obj["route"] = "univ_p_delai_paiement_edit";
    obj["data"] = dta;
