...
var table = $('#grid').DataTable({
    ajax: {
        url: Routing.generate('univ_p_enseignant_grille_list') + location.search, // json datasource
        type: "get",
    },
    "deferRender": true,
    "stateSave": true,
...
table.on("init.dt", function (e, settings) {
  ...
  CustomSearchDatatable.CustomSearchSelect(api, [4,2,3,5,7], []);
...
$(".bod").on("click", "._delete", function (e) {
    ...
    if (id) {
      var obj = new Object();
      obj["route"] = "univ_p_enseignant_grille_delete";
      obj["id"] = id;
      obj["token"] = token;
...
$(".bod").on("submit", ".form", function (e) {
    ...
    var selector = $(this);
    if ($(document.activeElement).hasClass("button-save-add-enseignant")) {
        //alert()
      var l = Ladda.create(document.querySelector(".button-save-add-enseignant"));
      var goto = 1;
    } else {
      var l = Ladda.create(document.querySelector(".button-save-enseignant"));
      var goto = 0;
    }
    l.start();
    var id = $("#test").data("enseignantId");
    var obj = new Object();
      obj["route"] = "univ_p_enseignant_grille_add";
      obj["data"] = dta;
      obj["parametre"] = null;
      obj["l"] = l;
...
$(".add").click(function (e) {
    $.ajax({
      type: "POST",
      url: Routing.generate("univ_p_enseignant_grille_add"),
  ...
  $("#grid tbody").on("click", ".cd_op", function () {
    $.ajax({
      type: "POST",
      url: Routing.generate("univ_p_enseignant_grille_edit", {
  ...
  $(".bod").on("submit", ".form_edit", function (e) {
    ...
    var id = $("#test").data("enseignantId");
    var l = Ladda.create(document.querySelector(".button-edit-enseignant"));
    l.start();
    var obj = new Object();
    var parametre = { id: id };
    obj["route"] = "univ_p_enseignant_grille_edit";
    obj["data"] = dta;
