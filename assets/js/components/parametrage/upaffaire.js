...
var table = $('#grid').DataTable({
    ajax: {
        url: Routing.generate('u_p_affaire_list'), // json datasource
        type: "get",
    },
    "deferRender": true,
    "stateSave": true,
...
table.on("init.dt", function (e, settings) {
  ...
  CustomSearchDatatable.CustomSearchSelect(api, [3,4], []);
...
$(".add").click(function (e) {
    $.ajax({
      type: "POST",
      url: Routing.generate("u_p_affaire_new"),
  ...
  $(".bod").on("submit", ".form", function (e) {
    ...
    if ($(document.activeElement).hasClass("button-save-add-affaire")) {
        //alert()
      var l = Ladda.create(document.querySelector(".button-save-add-affaire"));
      var goto = 1;
    } else {
      var l = Ladda.create(document.querySelector(".button-save-affaire"));
      var goto = 0;
    }
    l.start();
  ...
  $("#grid tbody").on("click", ".cd_op", function () {
    $.ajax({
      type: "POST",
      url: Routing.generate("u_p_affaire_edit", {
  ...
  $(".bod").on("submit", ".form_edit", function (e) {
    ...
    var id = $("#test").data("affaireId");
    var l = Ladda.create(document.querySelector(".button-edit-affaire"));
    l.start();
    var obj = new Object();
    var parametre = { id: id };
