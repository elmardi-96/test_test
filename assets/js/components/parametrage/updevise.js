...
var table = $('#grid').DataTable({
    ajax: {
        url: Routing.generate('u_p_devise_list'), // json datasource
        type: "get",
    },
    "deferRender": true,
    "stateSave": true,
...
$(".add").click(function (e) {
    $.ajax({
      type: "POST",
      url: Routing.generate("u_p_devise_new"),
  ...
  $(".bod").on("submit", ".form", function (e) {
    ...
    if ($(document.activeElement).hasClass("button-save-add-devise")) {
        //alert()
      var l = Ladda.create(document.querySelector(".button-save-add-devise"));
      var goto = 1;
    } else {
      var l = Ladda.create(document.querySelector(".button-save-devise"));
      var goto = 0;
    }
    l.start();
  ...
  $("#grid tbody").on("click", ".cd_op", function () {
    $.ajax({
      type: "POST",
      url: Routing.generate("u_p_devise_edit", {
  ...
  $(".bod").on("submit", ".form_edit", function (e) {
    ...
    var id = $("#test").data("deviseId");
    var l = Ladda.create(document.querySelector(".button-edit-devise"));
    l.start();
    var obj = new Object();
    var parametre = { id: id };
