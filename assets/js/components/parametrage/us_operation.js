...
var table = $('#grid').DataTable({
    ajax: {
        url: Routing.generate('us_operation_list'), // json datasource
        type: "get",
    },
    "deferRender": true,
    "stateSave": true,
...
$('#grid tbody').on('click', '.cd_op', function () {
    var url = Routing.generate('us_operation_edit', { 'id': $(this).closest('tr').attr('id') });
...
$('#_edit').click(function (e) {
    ...
    if (id) {
        var url = Routing.generate('us_operation_edit', { 'id': id });
        window.location.href = url;
        // console.log(url)
    } else {
    ...
    if (id) {
        swalWithBootstrapButtons.fire({
            ...
            if (result.value) {
                $.ajax({
                    type: 'DELETE',
                    url: Routing.generate('us_operation_delete', { 'id': id }),
                    success: function (result) {
                        ...
                        if (result.code == 200) {
                            ...
                            var url = Routing.generate('us_operation_index');
                            window.location.href = url;
