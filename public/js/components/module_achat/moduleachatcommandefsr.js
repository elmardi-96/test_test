...
var table = $('#grid').DataTable({
    ajax: {
        url: Routing.generate('commandefrs_list'),
        type: "get",
    },
    "orderCellsTop": true,
    "fixedHeader": true,
...
//}
    ...
    CustomSearchDatatable.CustomSearchText(api, [2 , 5 , 6 , 7 ], []);
    //CustomSearchDatatable.CustomSearchDate(api, [3 , 8], []);
    CustomSearchDatatable.CustomSearchSelect(api, [4, 9, 10 ,  11], []);
    CustomSearchDatatable.CustomSearchCheckbox(api, [1], []);
...
///*ajouter une class pour le recharche de chaque column de datatable */
//$('#grid > tfoot tr th .form-control').each(function (index, tr) {
//    $(this).addClass('scolumn_' + (index + 1));
//});
///*evenement datatable qui permet recupérer les données de chque input select ou bien date et réalise un miseajour sur datatable */
//$(".search_column").bind("keyup change", function () {
//    var scolumn = $(this).attr('class').split(' ').pop();
//    var column = scolumn.split('_')
//    table.columns(column[1]).search($(this).val()).draw();
//});
        ...
        success: function (result) {
            ...
            if (result.errors) {
               ...
               // alert(result.data.id);
                ...
                var url = Routing.generate('module_achat_commandefrs_show', {'id': result.data.id});
...
$('#_edit').click(function (e) {
    ...
    if (id) {
        var url = Routing.generate('module_achat_commandefrs_show', {'id': id});
        window.location.href = url;
    } else {
...
$('body').on('submit', '#module_achat_commandefrs_apply_transitions', function (e) {
    var form_name = $(this).attr('name');
    var dta = new FormData(this);
    var id = dta.get('_id');
    ...
    if (id) {
        swalWithBootstrapButtons.fire({
            ...
            confirmButtonText: "<i class='fa fa-ban'></i> Oui !",
            ...
            if (result.value) {
                $.ajax({
                    ...
                    processData: false,
                    contentType: false,
                    url: Routing.generate('module_achat_commandefrs_apply_transitions', {'id': id}),
                    success: function (result) {
                        if (result.code == 200) {
                            toastr.success(result.message.text, result.message.title, {timeOut: 4000})
                            location.reload();
...
$('body').on('submit', '#module_achat_commandefrs_apply_generer', function (e) {
    ...
    if (id) {
        $.ajax({
            type: 'POST',
            url: Routing.generate('module_achat_commandefrs_apply_generer', {'id': id}),
            success: function (result) {
                $('.actions').html(result);
...
$('.edit').on('submit', '.form', function (e) {
    ...
    $.ajax({
        type: 'POST',
        url: Routing.generate('module_achat_commandefrs_update', {id: id}),
        data: dta,
        processData: false,
        contentType: false,
        success: function (result) {
...
$('.edit').on('submit', '.detail_form_delete', function (e) {
    ...
    swalWithBootstrapButtons.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                type: 'DELETE',
                url: Routing.generate('module_achat_commandefrsdet_delete', {'id': id, 'token': token}),
                data: dta,
                processData: false,
                contentType: false,
                success: function (result) {
...
$('.edit').on('submit', '.detail_edit', function (e) {
    ...
    $.ajax({
        type: 'POST',
        url: Routing.generate('module_achat_commandefrsdet_edit_detail', {'id': id}),
        success: function (result) {
...
$('.edit').on('submit', '.detail_form_add_article', function (e) {
    ...
    $.ajax({
        type: 'POST',
        url: Routing.generate('module_achat_commandefrsdet_insert', {'id': id}),
        data: dta,
        processData: false,
        contentType: false,
        success: function (result) {
...
$('body').on('submit', '._fichiers', function (e) {
    ...
    $.ajax({
        type: 'POST',
        url: Routing.generate('module_achat_commandefrs_fichiers', {'id': id}),
        data: dta,
        processData: false,
        contentType: false,
        success: function (result) {
