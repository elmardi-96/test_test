...
var table = $('#grid').DataTable({
    ajax: {
        url: Routing.generate('u_x_banque_list') + location.search, // json datasource
        type: "get",
    },
    "deferRender": true,
    "stateSave": true,
...
$('.actions').on('submit', '.form_statut', function (e) {
    ...
    if (id) {
        $.ajax({
            type: 'POST',
            url: Routing.generate('u_x_banque_statut', {'id': id}),
            data: dta,
            processData: false,
            contentType: false,
...
$('#_edit').click(function (e) {
    ...
    if (id) {
        ...
        var url = Routing.generate('u_x_banque_edit', { 'id': id });
...
$('#_show').click(function (e) {
    ...
    if (id) {
        var url = Routing.generate('u_x_banque_show', {'id': id});
        window.location.href = url;
        // console.log(url)
    } else {
        toastr.warning("Veuillez Renseigner une ligne pour effectuer cette opération", "warning", {timeOut: 4000})
    ...
    if (id) {
        swalWithBootstrapButtons.fire({
            ...
            if (result.value) {
                $.ajax({
                    type: 'DELETE',
                    url: Routing.generate('u_x_banque_delete', {'id': id}),
                    success: function (result) {
                        console.log(result);
                        if (result.code == 200) {
                            table.ajax.reload();
