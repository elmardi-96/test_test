...
var table = $('#grid').DataTable({
    ...
    "columns": [
        ...
        {"orderable": false, "width": "100px", className: "colonne-gras", "targets": [9]},
...
table.on('init.dt', function (e, settings) {
    ...
    if (st != null) {
        ...
        $(".search_column2").val(st.columns[3].search.search);
        $(".search_column3 option[value='" + st.columns[2].search.search + "']").attr("selected", "selected");
        ...
        $(".search_column11 option[value='" + st.columns[7].search.search + "']").attr("selected", "selected");
    }
...
$('.search_column1_clear2').click(function () {
...
//                $(".new .table-detail > tbody tr").not(":last").remove();
                toastr.success(result.message.text, result.message.title, {timeOut: 4000})
                var url = Routing.generate('t_commandefrscab_show', {'id': result.data.id});
                window.location.href = url;
            }
        },
    ...
    if (id) {
        swalWithBootstrapButtons.fire({
            ...
            if (result.value) {
                $.ajax({
                    type: 'DELETE',
                    url: Routing.generate('t_achatdemandeinternecab_delete', {'id': id}),
                    success: function (result) {
                        console.log(result);
                        if (result.code == 200) {
                            table.ajax.reload();
