...
var table = $('#grid').DataTable({
    ajax: {
        url: Routing.generate('user_list'), // json datasource
        type: "get",
    },
    "deferRender": true,
    "stateSave": true,
...
$('.new').on('submit', '.form', function (e) {
    ...
    //console.log('here');
       ...
       $.ajax({
           type: 'POST',
           url: Routing.generate('user_insert'),
           data: dta,
           processData: false,
           contentType: false,
...
table.on('init.dt', function (e, settings) {
    ...
    $('.edit').on('submit', '.form', function (e) {
        ...
        $.ajax({
            type: 'POST',
            url: Routing.generate('user_update', { id: id }),
            data: dta,
            processData: false,
            contentType: false,
            success: function (result) {
        ...
        swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    $.ajax({
                        type: 'DELETE',
                        url: Routing.generate('use_delete', { 'id': id, 'token': token }),
                        data: dta,
                        processData: false,
                        contentType: false,
                        success: function (result) {
    ...
    $('.edit').on('submit', '.detail_form_add_article', function (e) {
        ...
        $.ajax({
            type: 'POST',
            url: Routing.generate('user_insert', { 'id': id }),
            data: dta,
            processData: false,
            contentType: false,
            success: function (result) {
    ...
    $('.statut_action').click(function (e) {
        ...
        if (id) {
            $.ajax({
                type: 'POST',
                url: Routing.generate('user_statut', { 'id': id }),
                success: function (result) {
                    $('.actions').html(result);
                    $(".actions").toggleClass('sidebar-open');
                }
    ...
    $('#show_action').click(function (e) {
        ...
        if (id) {
            ...
            var url = Routing.generate('user_show', { 'id': id });
    ...
    if (id) {
        swalWithBootstrapButtons.fire({
            ...
            if (result.value) {
                $.ajax({
                    type: 'DELETE',
                    url: Routing.generate('user_delete', {'id': id}),
                    success: function (result) {
                        console.log(result);
                        if (result.code == 200) {
                            table.ajax.reload();
...
/*         } else {
    ...
    $('#edit_action').click(function (e) {
        ...
        if (id) {
            ...
            var url = Routing.generate('user_edit', { 'id': id });
            window.location.href = url;
            // console.log(url)
        } else {
            toastr.warning("Veuillez Renseigner une ligne pour effectuer cette opération", "warning", { timeOut: 4000 })
