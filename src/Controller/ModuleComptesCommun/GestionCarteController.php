...
class GestionCarteController extends AbstractController {
    ...
    /**
     * @Route("/", name="compte_carte_index",   options = { "expose" = true }  , methods={"GET"})
     */

    public function compte_carte_index(Connection $connection,Breadcrumbs $breadcrumbs): Response 
    {  
        $operations = $this->appService->getOperations('_module_gestioncomptecomun', '_gestion_carte', '_detail', true);
        
        $dc = $this->appService->getDossierCourant();
        $entityManager = $this->getDoctrine()->getManager();
        $compteDossierCourantCarte = $entityManager->getRepository(PCompteBanque::class)->findOneByDossierAndType($dc->getId(),6);
        ...
        $total_sold_carte =  number_format($compteDossierCourantCarte->getMontantFinal(),2,","," ");
        $sold_total =  number_format($compteDossierCourantVirtuelle->getMontantFinal() + $compteDossierCourantCarte->getMontantFinal(),2,","," ");
        return $this->render('module_gestion_comptes/gestion_carte/index.html.twig',[
            'operations' => $operations,
            "total_sold__virtuelle" => $total_sold__virtuelle,
            "total_sold_carte" => $total_sold_carte,
     ...
     * 
     *
     * @Route("/list_index",options = { "expose" = true } , name="compte_carte_list")
     * 
     */
    public function compte_carte_list(Connection $connection) {
        ...
        // dd($action);
        $operations = $this->appService->getOperations('_module_gestioncomptecomun', '_gestion_carte', '_new', true);
        ...
        foreach ($Depot_Retrait as $key => $DR) {
                ...
                $entityManager = $this->getDoctrine()->getManager();
                $compteDossierCourantCarte = $entityManager->getRepository(PCompteBanque::class)->findOneByDossierAndType($DR['id_dossier'],6);
                $compteDossierCourantVirtuelle = $entityManager->getRepository(PCompteBanque::class)->findOneByDossierAndType($DR['id_dossier'],4);
                if($compteDossierCourantCarte) {

                    $action =  $this->renderView('module_gestion_comptes/gestion_carte/actions.html.twig',[ 'operations' => $operations , 'id' => $DR['id_dossier']]);
                    ...
                    $nestedData[] = $compteDossierCourantCarte ? $compteDossierCourantCarte->getDescription() : "";
                    $nestedData[] = $compteDossierCourantCarte ? $compteDossierCourantCarte->getMontantFinal() : "";
                    $nestedData[] = $action;
    ...
    /**
     * @Route("/new/{dossier}", name="compte_carte_new", options = { "expose" = true } , methods={"GET","POST"})
     */
    public function new(PDossier $dossier, Connection $connection,Request $request, ValidatorInterface $validator, Breadcrumbs $breadcrumbs, TrTransactionRepository $TrTransactionRepository ,UGeneralOperationRepository $operationRepository): Response {
        ...
        $dc = $this->appService->getDossierCourant();
        $entityManager = $this->getDoctrine()->getManager();
        $compteBydossier = $entityManager->getRepository(PCompteBanque::class)->findOneByDossierAndType($dossier->getId(),6);
        $compteDossierCourantCarte = $entityManager->getRepository(PCompteBanque::class)->findOneByDossierAndType($dc->getId(),6);
        $compteDossierCourantVirtuelle = $entityManager->getRepository(PCompteBanque::class)->findOneByDossierAndType($dc->getId(),4);
        // $data = [
        //     'compte' => [
        ...
        // ];
        $html =  $this->render('module_gestion_comptes/gestion_carte/modal_new.html.twig', [
                'dossier' => $dossier,
                'compte' => $compteBydossier,
                'compteDossierCourantCarte' => $compteDossierCourantCarte,
    ...
    /**
     * @Route("/store", name="compte_carte_store", options = { "expose" = true } , methods={"GET","POST"})
     */
    public function store(Request $request): Response {
        ...
        $compteBydossierCaisse = $entityManager->getRepository(PCompteBanque::class)->findOneByDossierAndType($dossier->getId(),6);
        $compteBydossierVirtuelle = $entityManager->getRepository(PCompteBanque::class)->findOneByDossierAndType($dossier->getId(),4);
        $compteDossierCourantPhysique = $entityManager->getRepository(PCompteBanque::class)->findOneByDossierAndType($dc->getId(),6);
        $compteDossierCourantVirtuelle = $entityManager->getRepository(PCompteBanque::class)->findOneByDossierAndType($dc->getId(),4);
        ...
        $operationToPhysique->setPiece(
            $entityManager->getRepository(UpPiece::class)->find(44) // mouvement carte
        );
