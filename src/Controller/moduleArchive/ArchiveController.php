...
class ArchiveController extends AbstractController {
    ...
    /**
     * @Route("/{pro_id}/archive", name="archive")
     */
    public function archive(Request $request, Breadcrumbs $breadcrumbs, $pro_id ): Response {
        ...
        $breadcrumbs->addRouteItem("Projets", "module_archive_projet_index");
        $breadcrumbs->addRouteItem($projet->getIntitule(), "app");
        return $this->render('module_archive/archive/index.html.twig',array('projet_id'=>$projet));

    }

    /**
     * Grid action
     * @Route("/module_archive_select_detail_by_id", name="module_archive_select_detail_by_id")
     * 
     */
    ...
    /**
     * Grid action
     * @Route("/module_archive_get_root_node_obj", name="module_archive_get_root_node_obj")
     * 
     */
    ...
    /**
     * Grid action
     * @Route("/module_archive_get_root_node_obj_oper", name="module_archive_get_root_node_obj_oper")
     * 
     */
    public function get_root_node_obj_oper(Request $request) {
        ...
        /**
     * Grid action
     * @Route("/module_archive_get_node", name="module_archive_get_node")
     * 
     */
...
//        $items = array();
    ...
    /**
     * Grid action
     * @Route("/module_archive_get_root_node", name="module_archive_get_root_node")
     * 
     */
...
//+ array('state' => array('opened' => true))
    ...
    /**
     * Grid action
     * @Route("/module_archive_get_root", name="module_archive_get_root")
     * 
     */
    ...
    /**
     * Grid action
     * @Route("/module_archive_select_detail_by_id_without_size", name="module_archive_select_detail_by_id_without_size")
     * 
     */
    ...
    /**
     * Grid action
     * @Route("/module_archive_get_node_id", name="module_archive_get_node_id")
     * 
     */
    public function get_node_id(Request $request) {
        ...
        $query = $entityManager->createQuery('
        SELECT c.TreeId as id , c.text, c.user , c.dateCreation ,c.icon ,c.type
        FROM App\Entity\ArcTree c where c.parent = :id and c.status <> :disable order by c.type asc
        ')
        ->setParameter('id', $id)
        ->setParameter('disable', "disable");
        $categorias = $query->getArrayResult();
    ...
    /**
     * Grid action
     * @Route("/module_archive_save_post", name="module_archive_save_post")
     * 
     */
    ...
    /**
     * Grid action
     * @Route("/module_archive_save_file", name="module_archive_save_file")
     * 
     */
    ...
    /**
     * Grid action
     * @Route("/module_archive_importe_file", name="module_archive_importe_file")
     * 
     */
    public function importe_file(Request $request, ArcTreeRepository $ArcTreeRepository) {
        ...
        if (is_array($parents)) {
            ...
            $target_dir = "./root/".$projet->getIntitule().'/'.str_replace('//', '/', $str1) . $_SESSION['data']['text'] . "/";
        } else {

            $target_dir = "./root/".$projet->getIntitule().'/root/';
        }
       ...
       // die($_FILES["file"]["type"]);
        ...
        //dump($obj);die();
        $SaveTree->setProjet($obj->getProjet());
    ...
    /**
     * Grid action
     * @Route("/module_archive_get_url_image", name="module_archive_get_url_image")
     * 
     */
     ...
     * Export to PDF
     * 
     * @Route("/module_archive_update_hide_doc", name="module_archive_update_hide_doc")
     */
    ...
    /**
     * Grid action
     * @Route("/module_archive_select_node_id", name="module_archive_select_node_id")
     * 
     */
     ...
     * Export to PDF
     * 
     * @Route("/module_archive_create_node", name="module_archive_create_node")
     */
...
//        die();
        ...
        foreach ($arr2[0] as $key1 => $value1) {
            // echo $value1;die();
            $em = $this->getDoctrine()->getManager();
            $re1 = $em->getRepository(ArcTree::class)->findOneBy(['TreeId' => $value1]);
            $str1 .= $re1->getText() . '/';
        ...
        @mkdir($str1 . $request->query->get('text'), 0777, true);
...
// actually executes the queries (i.e. the INSERT query)
     ...
     * Export to PDF
     * 
     * @Route("/module_archive_delete_node_file", name="module_archive_delete_node_file")
     */
    public function delete_node_file(Request $request, ArcTreeRepository $ArcTreeRepository) {
        ...
        //var_dump($str1);die();
        $em = $this->getDoctrine()->getManager();
        $re1 = $ArcTreeRepository->getTreeById($request->query->get('id'));

        @unlink(str_replace('//', '/', $str1) . $re1[0]['text']);
     ...
     * Export to PDF
     * 
     * @Route("/module_archive_update_node", name="module_archive_update_node")
     */
    ...
    FROM App\Entity\ArcTree c where c.parent = :parent and c.text = :text or c.text = :text order by c.type asc'
		...
		//echo $request->request->get('projet_id'); die();
		$Projet = $this->getDoctrine()->getRepository(ArcProjet::class)->find($request->query->get('projet_id'));
        $str1 = "./root/".$Projet->getIntitule()."/";
        $arr1 = array_reverse($request->query->get('parent'));
        $arr1[] = array_slice($arr1, 1, count($arr1));
        ...
        $em = $this->getDoctrine()->getManager();
        $re1 = $ArcTreeRepository->getTreeById($request->query->get('id'));
        // echo str_replace('//', '/', $str1) . $re1[0]['text'];echo str_replace('//', '/', $str1) . $request->query->get('text');die();
        @rename(str_replace('//', '/', $str1) . $re1[0]['text'], str_replace('//', '/', $str1) . $request->query->get('text'));
     ...
     * Export to PDF
     * 
     * @Route("/module_archive_delete_node_folder", name="module_archive_delete_node_folder")
     */
    public function delete_node_folder(Request $request, ArcTreeRepository $ArcTreeRepository) {
        ...
        @rmdir(str_replace('//', '/', $str1) . $re1[0]['text']);
    ...
    /**
     * Grid action
     * @Route("/module_archive_copy_node", name="module_archive_copy_node")
     * 
     */
     ...
     * Export to PDF
     * 
     * @Route("/module_archive_compress_node", name="module_archive_compress_node")
     */
    public function compress_node(Request $request, ArcTreeRepository $ArcTreeRepository) {
        ...
        $re1 = $ArcTreeRepository->zipData( str_replace('//', '/', $str1) . $request->query->get('text'), str_replace('//', '/', $str1) . $request->query->get('text') . '.zip',$Projet->getIntitule());
        if (file_exists(str_replace('//', '/', $str1) . $request->query->get('text') . '.zip')) {
     ...
     * Export to PDF
     * 
     * @Route("/module_archive_compress_node_delete_after_load", name="module_archive_compress_node_delete_after_load")
     */
