...
class AppService {
    ...
    public function __construct(SessionInterface $session, Environment $twig, Breadcrumbs $breadcrumbs) {
    ...
    public function getDossiers() {
     //   dump($this->session);
       // die;
        if (!$this->session->get('dossiers')) {
            throw $this->createNotFoundException('Dossiers Not Existes');
        } else {
            return $this->session->get('dossiers');
        }
    }

    public function getDossiersIds($modulePrefix) {
//        dump($this->session->get('dossiers'));
//        die();
        $ids = array();
        foreach ($this->session->get('dossiers') as $key => $dossier) {
    ...
    public function getDossierCourant() {
        if (!$this->session->get('dossierCourant')) {
            throw $this->createNotFoundException('Dossier courant non trouvé');
        } else {
            return $this->session->get('dossierCourant');
        }
    }
